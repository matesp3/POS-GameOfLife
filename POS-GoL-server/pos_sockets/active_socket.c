...
void active_socket_init(struct active_socket* self) {
    ...
    self->end_message = "sock:end"; // dohodnute medzi klientom a serverom
...
void active_socket_start_reading(struct active_socket* self, pthread_cond_t* cond_other_thread_start) {
#define BUFFER_LENGTH 100
    ...
    pthread_cond_signal(cond_other_thread_start);
    pthread_mutex_unlock(&self->mutex_reading);
